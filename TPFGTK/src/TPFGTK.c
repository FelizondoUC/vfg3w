...
GtkButton *Capturar;
//El texto que va a ir cambiando;
GtkWidget *arriba_captura;
GtkWidget *abajo_captura;
GtkWidget *izquierda_captura;
GtkWidget *derecha_captura;
GtkEntry *Nombre_1;
GtkEntry *Nombre_2;
...
int capturai=0;
int capturaj=0;
void funcion_comer_varios(int i,int j){
	int izquierda= board[i][j-1];
	(izquierda>6) ? izquierda-6: 0;
	int derecha=board[i][j+1];
	(derecha>6) ? derecha-6: 0;
	int arriba=board[i-1][j];
	(arriba>6) ? arriba-6: 0;
	int abajo=board[i+1][j];
	(abajo>6) ? abajo-6: 0;
	capturai=i;
	capturaj=j;
	printf("Hola i %d \n",izquierda);
	...
	if(izquierda!='\0'){
		char buffer[50];
		snprintf(buffer,sizeof(buffer),"Izquierda %d",izquierda);
		gtk_label_set_text(GTK_LABEL(izquierda_captura), buffer);
	}else{
		char buffer[50];
		snprintf(buffer,sizeof(buffer),"Izquierda 0");
		gtk_label_set_text(GTK_LABEL(izquierda_captura), buffer);
		}
	if(derecha!='\0'){
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Derecha %d",derecha);
			gtk_label_set_text(GTK_LABEL(derecha_captura), buffer);
		}else{
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Derecha 0");
			gtk_label_set_text(GTK_LABEL(derecha_captura), buffer);
			}
	if(arriba!='\0'){
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Arriba %d",arriba);
			gtk_label_set_text(GTK_LABEL(arriba_captura), buffer);
		}else{
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Arriba 0");
			gtk_label_set_text(GTK_LABEL(arriba_captura), buffer);
			}
	if(abajo!='\0'){
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Abajo %d",abajo);
			gtk_label_set_text(GTK_LABEL(abajo_captura), buffer);
		}else{
			char buffer[50];
			snprintf(buffer,sizeof(buffer),"Abajo 0");
			gtk_label_set_text(GTK_LABEL(abajo_captura), buffer);
			}

}
int botoncapmultipleFLAG=0;
int sumcapmultiple=0;
void capturar(GtkToggleButton *toggle_button, gpointer data) {
	int i=capturai;
	int j=capturaj;
	...
	if(gtk_toggle_button_get_active(toggle_button)){
		botoncapmultipleFLAG++;
		switch (num) {
		case 1:
			arriba=board[i-1][j];
			sumcapmultiple= sumcapmultiple+arriba;
			break;
		case 2:
			abajo=board[i+1][j];
			sumcapmultiple= sumcapmultiple+abajo;
			break;
		case 3:
			izquierda= board[i][j-1];
			sumcapmultiple= sumcapmultiple+izquierda;
			break;
		case 4:
			derecha=board[i][j+1];
			sumcapmultiple= sumcapmultiple+derecha;
			break;

		}
		snprintf(buffer, sizeof(buffer), "La suma actual es: %d", sumcapmultiple);
		gtk_label_set_text(GTK_LABEL(suma_actual), buffer);
	}else{
		botoncapmultipleFLAG--;
		switch (num) {
		case 1:
...
arriba_captura=GTK_WIDGET(gtk_builder_get_object(builder, "arriba_captura"));;
abajo_captura=GTK_WIDGET(gtk_builder_get_object(builder, "abajo_captura"));
izquierda_captura=GTK_WIDGET(gtk_builder_get_object(builder, "izquierda_captura"));
derecha_captura=GTK_WIDGET(gtk_builder_get_object(builder, "derecha_captura"));
...
suma_actual= GTK_WIDGET(gtk_builder_get_object (builder, "suma_actual"));
...
g_signal_connect(Botones[4][4], "clicked", G_CALLBACK(actualizar_boton25),
		NULL);
g_signal_connect(Arriba, "clicked", G_CALLBACK(capturar), GINT_TO_POINTER(1));
g_signal_connect(Abajo, "clicked", G_CALLBACK(capturar), GINT_TO_POINTER(2));
g_signal_connect(Izquierda, "clicked", G_CALLBACK(capturar), GINT_TO_POINTER(3));
g_signal_connect(Derecha, "clicked", G_CALLBACK(capturar), GINT_TO_POINTER(4));
gtk_widget_show_all(Ventana_Principal);
