...
int comeAdyacente(int x, int y, int tablero, int player)
{
	...
	if (count > 2) {
		...
		//Es HUMANO ?
			...
			/*do {
				printf("\nIntroduzca dados a comer (2 o más dados):");
				scanf("%d", &dadosAComer);
			} while (dadosAComer < 2 || dadosAComer > 4);*/


			/*do {
				printf("\nDados a comer elegidos: %d de %d", adyElegidos, dadosAComer);
				printf("\nElegir adyacente a comer (Fila Columna) (-1 para salir): ");
				scanf("%d %d", &adyx, &adyy);
				for (int i = 0; i < 4; i++)
					if (hayDadosAdy[i] && adyacentes[i][0] == adyx
							&& adyacentes[i][1] == adyy) {
						// Controlar que no pase 6 y que el dado adyacente no sea 6
						if (tablero == MAINBOARD && board[adyx][adyy] != 6 && board[adyx][adyy] != 12)
							tempDado = board[adyx][adyy] + (board[adyx][adyy] < 6 ? 0: -6);
						else if (tablero == MIRRORBOARD && mirror[adyx][adyy] != 6 && mirror[adyx][adyy] != 12)
							tempDado = mirror[adyx][adyy]+ (mirror[adyx][adyy] < 6 ? 0: -6);
						else {
			...
			} while (adyElegidos < dadosAComer && ( (adyx != -1 && adyy != -1) || adyElegidos < 2) );*/
